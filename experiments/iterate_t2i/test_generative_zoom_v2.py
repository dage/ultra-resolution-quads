...
class MockComfyUIClientV2:
    def __init__(self, server_address, client_id, **kwargs):
        ...
        print(f"[MockV2] Initialized for {server_address}")

    def upload_image(self, filepath, **kwargs):
        print(f"[MockV2] Uploading {filepath}")
    ...
    def queue_prompt(self, workflow):
        if "100" in workflow:
            ...
            print(f"[MockV2] Detected Context Image: {ctx_name}")
        ...
        if "18" in workflow:
            ...
            print(f"[MockV2] Queueing Img2Img with input: {img_name}")
        else:
            self.last_prompt_type = "t2i"
            print("[MockV2] Queueing T2I")
...
DATASET_ID = "generative_infinity_zoom_v2"
...
def generate_html_report(report_text, score, img_stitched, img_crop):
    html = f"""
    <html>
    <head><title>Generative Zoom V2 Analysis</title></head>
    <body style="font-family: sans-serif; max-width: 800px; margin: 0 auto; padding: 20px;">
        <h1>Generative Zoom V2 Analysis</h1>
    ...
    """
    with open("artifacts/gen_zoom_v2_debug/report.html", "w") as f:
        f.write(html)
    print("\n[Report] Generated report.html in artifacts/gen_zoom_v2_debug/")

def stitch_center_quad_l2():
    # L2 is 4x4. Center is (1,1) to (2,2).
    tiles = {}
    for x in [1, 2]:
        for y in [1, 2]:
...
def run_test(args):
    try:
        ...
        # Only render the center 4 tiles of L2
        tasks_l2 = [
        ...
        debug_dir = Path("artifacts/gen_zoom_v2_debug")
        debug_dir.mkdir(parents=True, exist_ok=True)
        stitched_l2.save(debug_dir / "level_2_center.png")
        
        # Crop seam center of L2
        # Stitched L2 is 1024x1024. Center 512,512.
...
def main():
    ...
    if args.mock:
        ...
        with patch('backend.comfyui_client.ComfyUIClient', side_effect=MockComfyUIClientV2):
            run_test(args)
    else:
