...
DATASET_ID = "generative_infinity_zoom_v8"
...
def generate_html_report(report_text, score, img_stitched, img_crop):
    html = f"""
    <html>
    <head><title>Generative Zoom V8 Analysis</title></head>
    <body style="font-family: sans-serif; max-width: 800px; margin: 0 auto; padding: 20px;">
        <h1>Generative Zoom V8 Analysis</h1>
        <p><strong>Experiment:</strong> 512px tiles + neighbor context + parent fill + Differential Diffusion (denoise 0.3).</p>
    ...
    """
    out_dir = Path("artifacts/gen_zoom_v8_debug")
    out_dir.mkdir(parents=True, exist_ok=True)
    with open(out_dir / "report.html", "w") as f:
        f.write(html)
    print("\n[Report] Generated report.html in artifacts/gen_zoom_v8_debug/")
...
def run_test(args):
    ...
    debug_dir = Path("artifacts/gen_zoom_v8_debug")
