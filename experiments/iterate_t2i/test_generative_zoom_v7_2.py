...
DATASET_ID = "generative_infinity_zoom_v7_2"
...
def generate_html_report(report_text, score, img_stitched, img_crop):
    html = f"""
    <html>
    <head><title>Generative Zoom V7-2 Analysis</title></head>
    <body style="font-family: sans-serif; max-width: 800px; margin: 0 auto; padding: 20px;">
        <h1>Generative Zoom V7-2 Analysis</h1>
        <p><strong>Experiment:</strong> 512px tiles + parent-filled context + Differential Diffusion (denoise 0.15).</p>
    ...
    """
    out_dir = Path("artifacts/gen_zoom_v7_2_debug")
    out_dir.mkdir(parents=True, exist_ok=True)
    with open(out_dir / "report.html", "w") as f:
        f.write(html)
    print("\n[Report] Generated report.html in artifacts/gen_zoom_v7_2_debug/")
...
def run_test(args):
    ...
    debug_dir = Path("artifacts/gen_zoom_v7_2_debug")
