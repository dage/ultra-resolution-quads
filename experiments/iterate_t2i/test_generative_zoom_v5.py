...
class MockComfyUIClientV5:
    def __init__(self, server_address, client_id, **kwargs):
        ...
        print(f"[MockV5] Initialized for {server_address}")

    def upload_image(self, filepath, **kwargs):
        print(f"[MockV5] Uploading {filepath}")
    ...
    def queue_prompt(self, workflow):
        if "100" in workflow:
            ...
            print(f"[MockV5] Detected Context Image: {ctx_name}")
        
        if "101" in workflow:
            mask_name = workflow["101"]["inputs"].get("image")
            self.last_mask_image = self.uploaded_files.get(mask_name)
            print(f"[MockV5] Detected Mask Image: {mask_name}")
            
        if "102" in workflow: # SetLatentNoiseMask
            self.last_prompt_type = "inpainting"
            print("[MockV5] Queueing Inpainting")
        else:
            self.last_prompt_type = "t2i"
            print("[MockV5] Queueing T2I")
    ...
    def get_image_data(self, image_ref):
        if self.last_prompt_type == "t2i":
            ...
            if not self.last_input_image or not Path(self.last_input_image).exists():
                return Image.new("RGB", (256, 256)).tobytes()
            with Image.open(self.last_input_image) as input_img:
                output = input_img.resize((256, 256), Image.Resampling.NEAREST)
...
DATASET_ID = "generative_infinity_zoom_v5"
...
def generate_html_report(report_text, score, img_stitched, img_crop):
    html = f"""
    <html>
    <head><title>Generative Zoom V5 Analysis</title></head>
    <body style="font-family: sans-serif; max-width: 800px; margin: 0 auto; padding: 20px;">
        <h1>Generative Zoom V5 Analysis</h1>
        <p><strong>Experiment Note:</strong> V5 uses <strong>Inpainting/Outpainting</strong> (SetLatentNoiseMask). The context image includes neighbors and the parent crop. The center 512x512 is masked (Inpaint), while the border neighbors are frozen (Keep). This forces the model to generate the tile blended perfectly into the neighbors.</p>
    ...
    """
    with open("artifacts/gen_zoom_v5_debug/report.html", "w") as f:
        f.write(html)
    print("\n[Report] Generated report.html in artifacts/gen_zoom_v5_debug/")
...
def run_test(args):
    try:
        ...
        debug_dir = Path("artifacts/gen_zoom_v5_debug")
        debug_dir.mkdir(parents=True, exist_ok=True)
        stitched_l2.save(debug_dir / "level_2_center.png")
        
        # Crop seam center of L2
        # Stitched L2 is 512x512. Center 256,256.
        # Crop 128x128 around center (192, 192) to (320, 320).
...
def main():
    ...
    if args.mock:
        ...
        with patch('backend.comfyui_client.ComfyUIClient', side_effect=MockComfyUIClientV5):
            run_test(args)
    else:
