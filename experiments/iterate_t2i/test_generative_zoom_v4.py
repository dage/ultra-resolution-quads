...
class MockComfyUIClientV4:
    def __init__(self, server_address, client_id, **kwargs):
        ...
        print(f"[MockV4] Initialized for {server_address}")

    def upload_image(self, filepath, **kwargs):
        print(f"[MockV4] Uploading {filepath}")
    ...
    def queue_prompt(self, workflow):
        if "100" in workflow:
            ...
            print(f"[MockV4] Detected Context Image: {ctx_name}")
        if "18" in workflow:
            ...
            print(f"[MockV4] Queueing Img2Img with input: {img_name}")
        else:
            self.last_prompt_type = "t2i"
            print("[MockV4] Queueing T2I")
...
DATASET_ID = "generative_infinity_zoom_v4"
...
def generate_html_report(report_text, score, img_stitched, img_crop):
    html = f"""
    <html>
    <head><title>Generative Zoom V4 Analysis</title></head>
    <body style="font-family: sans-serif; max-width: 800px; margin: 0 auto; padding: 20px;">
        <h1>Generative Zoom V4 Analysis</h1>
        <p><strong>Experiment Note:</strong> V4 tests "Neighbor Context". Instead of just the parent, the context image includes the already-rendered Left, Top, and Top-Left tiles (plus the current input guidance). This enforces pixel-perfect continuity at the boundaries during generation.</p>
    ...
    """
    with open("artifacts/gen_zoom_v4_debug/report.html", "w") as f:
        f.write(html)
    print("\n[Report] Generated report.html in artifacts/gen_zoom_v4_debug/")
...
def run_test(args):
    try:
        ...
        # Requesting center tiles will recursively trigger dependencies (neighbors)
        tasks_l2 = [
        ...
        debug_dir = Path("artifacts/gen_zoom_v4_debug")
...
def main():
    ...
    if args.mock:
        ...
        with patch('backend.comfyui_client.ComfyUIClient', side_effect=MockComfyUIClientV4):
            run_test(args)
    else:
